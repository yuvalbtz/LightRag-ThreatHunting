...
async def query_stream(request: ChatRequest):
    ...
    try:
        ...
        print(f"Response type: {type(response)}")  # Debug log
        print(
            f"Response content: {response[:100] if isinstance(response, str) else 'generator'}"
        )  # Debug log
        ...
        print(f"Error in query_stream: {str(e)}")  # Debug log
        raise HTTPException(status_code=500, detail=str(e))
